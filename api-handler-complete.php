<?php
/**
 * Complete API Handler for GoDaddy Hosting
 * This file handles all API requests including authentication for admin login
 */
...
switch ($apiPath) {
    case '/packages':
        if ($requestMethod === 'GET') {
            echo json_encode(getPackagesFromDB(), JSON_UNESCAPED_SLASHES);
        } else {
    ...
    case '/destinations':
        if ($requestMethod === 'GET') {
            echo json_encode(getDestinationsFromDB(), JSON_UNESCAPED_SLASHES);
        } else {
            http_response_code(405);
            echo json_encode(['error' => 'Method not allowed']);
        }
...
function handleLogin() {
    ...
    if (!$conn) {
        // Fallback to hardcoded admin credentials if database unavailable
...
function verifyJWT($token) {
    // Simplified JWT verification (in production, use a proper JWT library)
    $parts = explode('.', $token);
    if (count($parts) !== 3) {
        return false;
    }
